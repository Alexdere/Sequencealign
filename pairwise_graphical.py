...
def create_matrix(sequence1, sequence2, gap):  
    mat = [[0] * (len(sequence2) + 2) for i in range(len(sequence1) + 2)] 
...
def backtrack(sequence1, sequence2, mat, gap): #need to add code that checks if the path has the lowest possible score, and finds multiple pathways if there
    ...
    while i > 0 and j > 0:
        #path.append([i+1,j+1])
        #print(path)
        current = mat[i+1][j+1]  # +1 due to the extra row and column in matrix
        diagonal = mat[i][j]
        up = mat[i][j+1]
        left = mat[i+1][j]
